...
ESP32-S3 / ESP32-C6 firmware for a desk-side Codex usage monitor. Each
...
- Connects by name (`"Clawdmeter"`) on first run, caches resolved MAC at `~/.config/Codex-usage-monitor/ble-address`. ESP32 BLE addresses are factory-burned per-chip, so swapping any board invalidates the cache.
