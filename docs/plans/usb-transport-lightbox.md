...
## Phase 4 後續:全螢幕 GIF 動畫的「由上往下逐條刷新」sweep(2026-07-10)

**現象**:使用者反映滿版播放的 GIF(尤其 `03_its_fine.gif` 遠景切近景那種畫面內容大幅變動的幀)看得到明顯的由上往下逐條刷新,不像一般 GIF 播放器那樣整幀瞬間切換。

**根因鏈(已查證)**:
1. `firmware/src/main.cpp:297-298` 用 `LV_DISPLAY_RENDER_MODE_PARTIAL`,`BUF_LINES=40`(`main.cpp:32`),480 高的畫面切成 12 段個別 flush。
2. 底層 `Arduino_ESP32QSPI` 驅動固定把任何寫入再切成 1024 像素一批的 DMA burst(`ESP32QSPI_MAX_PIXELS_AT_ONCE=1024`,vendored `Arduino_ESP32QSPI.h:9`),跟 LVGL 這邊設多大的 buffer 無關。
3. QSPI clock 原本是驅動預設值 40MHz(`ESP32QSPI_FREQUENCY`,同檔案),沒人動過。
4. 面板是 CO5300,查證過官方 datasheet 跟 Waveshare 這片板子的 pinout(`docs.waveshare.com/ESP32-S3-Touch-AMOLED-2.16`):**CO5300 晶片本身有 TE(Tearing Effect)輸出腳位這個功能,但 Waveshare 這片板子完全沒有把它接出來**——pinout 只有 `LCD_CS`/`QSPI_SIO0-3`/`QSPI_SCL`/`LCD_RESET` 七條線,`firmware/src/boards/waveshare_amoled_216/board.h` 也只定義這幾條。代表「軟體同步面板刷新時機」這條教科書正解,在這片板子上**硬體層級就不可行**,不是韌體沒接。
5. 因此 sweep 的根本原因是:面板自己的 GRAM 刷新跟我們寫入 GRAM 的時機完全脫鉤(沒有 TE 同步),而每次 GIF 動畫換幀,不管軟體怎麼切 buffer,都要把整張 480×480(460,800 bytes)寫進 GRAM,這段寫入的物理時間長到人眼看得出來由上往下的漸進效果——寫入量才是關鍵,不是切幾段或時脈多快。

**已做但只有部分效果**:QSPI clock 40MHz→80MHz(`firmware/src/boards/waveshare_amoled_216/display.cpp:28`,Waveshare 官方對這顆 CO5300 面板的參考韌體規格值,已查證)。傳輸時間理論砍半,實機盯著看:確實變快,但整幀內容大幅變動的那幾幀(遠景切近景)依然看得出漸進刷新——這條路只是縮短 sweep 的時間窗,沒有消除,因為寫入總量沒變。

**原記載「排除的方向」有誤,已用實機數字更正(2026-07-10 稍晚)**:當時判斷「GIF 素材改成 480×480 不會有幫助」是錯的——判斷依據只算了 QSPI 頻寬(傳輸 460,800 bytes 理論 ~11.5ms),沒算到 LVGL 軟體縮放本身的運算成本。實際插測(`my_flush_cb` 包 `millis()` 量測純 QSPI 寫入時間、`loop()` 包 `lv_timer_handler()` 量測整個 render+flush 週期,兩者相減):`05_minion_ahh.gif`(240×240 素材,COVER 縮放到 480×480)穩定態每幀實測約 226ms,其中 QSPI 寫入只佔 ~46ms(12 段 flush 加總),**其餘 ~170ms(約 79%)花在 LVGL 自己的軟體縮放/合成,不在我們的 QSPI 寫入函式裡**——這片 MCU 沒有硬體 2D 縮放器,所有 nearest-neighbor 縮放都是 CPU 逐 pixel 算的,遠比想像中貴。查證 `lv_draw_image.c`:LVGL 對「有縮放」(`scale_x/scale_y != LV_SCALE_NONE`)跟「無縮放」明確走不同程式碼路徑;`lv_image.c` 的 `update_align()` 算 COVER 縮放係數用 `obj_width * LV_SCALE_NONE / img->w`,若素材尺寸跟 widget 尺寸完全一致(如 480×480 素材配 480×480 widget),算出來的縮放係數精確等於 `LV_SCALE_NONE`(256),會直接命中免縮放的快速路徑。實機驗證:把 `05_minion_ahh.gif` 用 `ffmpeg -vf scale=480:480:flags=neighbor` 轉成 480×480 素材測試,同一支 GIF 的 `lv_timer_handler()` 單次呼叫時間從平均 ~216ms 掉到量不出來(372 次呼叫 2 秒內總計 0ms,低於 `millis()` 的量測解析度)——這是目前找到單一影響最大的改法,遠大於 QSPI 80MHz 或 dirty-rect diff 帶來的效果。

**過程中另外抓到一個真的 bug(2026-07-10 稍晚,已修)**:`turbobuf` 先前為了效能搬到 internal SRAM(見上方效能調查段落),當時是照 240×240 素材算的(~82KB,SRAM 裝得下)。素材真的換成 480×480 後 `turbobuf` 漲到 `480×480 + TURBO_BUFFER_SIZE ≈ 255KB`,這片晶片 internal SRAM 總共只有 320KB,扣掉系統其他配置後裝不下,`gif_player_open()` 直接配置失敗、GIF 完全沒有真的解碼——早期用 ffmpeg 測試 480×480 素材時量到的「0ms」其實是這個 bug 造成的假陽性(沒真的在解碼,`lv_timer_handler()` 自然量不出耗時),不是快速路徑本身有這麼誇張。修法:`sram_calloc()` 改成 internal SRAM 配置失敗時 fallback 回 PSRAM(`firmware/src/gif_player.cpp`),不再無條件失敗。修完後用使用者實際的 480×480 素材(非測試檔)重新量測,真實數字:`decode` 時間隨像素量等比例漲(240×240 的 ~12-15ms → 480×480 的 ~40-52ms,4 倍像素、4 倍解碼時間,預期內、無法避免),但整個 frame 週期(decode+diff+redraw)從原本 240×240 COVER 縮放的 ~226ms 降到 **82-91ms,已經落在或接近各自的幀預算內**(`03_fine.gif` 84ms/90ms、`04_confused.gif` 86-91ms/100ms、`05_minion_ahh.gif` 82-86ms/80ms)——不是完全免費,但從「2.8 倍慢」變成「接近即時」,改善幅度依然遠大於 QSPI 80MHz 或 dirty-rect diff 單獨帶來的效果。

**代價**:480×480 素材檔案量比 240×240 大,但不是單純 4 倍——使用者自己重新輸出(非本文件 ffmpeg 直接轉檔的版本)壓到六支素材合計 2.6MB,完整放進現有 3.375MB SPIFFS 分區還有約 700KB 餘裕,不需要再擴分區。ffmpeg 直接轉檔(未額外壓縮)的版本明顯大更多(三支 GIF 加一張 PNG 合計約 5.9MB,gifsicle `--colors=255 --lossy=80` 壓完仍約 5.2MB),超出分區——代表輸出工具/壓縮設定對檔案大小影響很大,同解析度不代表同檔案量,以後再加素材要注意這點,不能只看解析度估算空間。

**下一步(待實作)**:「差分後只重繪真的變動的區域」,滿版顯示範圍不動,但透過縮小每幀實際要傳輸的量來壓 sweep:

1. `gif_player.cpp` 的 `play_frame()` decode 新一幀前,先保留上一幀 RGB565 內容的副本(240×240×2 bytes,PSRAM,新增一塊小 buffer)
2. decode 完新一幀後,逐 pixel 比對新舊兩份,算出真正變動像素的 bounding box(240×240 素材座標系)
3. 把這個 bbox 依目前的 2x COVER 縮放映射到 480×480 輸出座標,呼叫 `lv_obj_invalidate_area()` 只標記這塊區域為髒,取代現在無條件的 `lv_obj_invalidate(gif_img)`(整個 widget)
4. LVGL 的 PARTIAL render 本來就是照髒區域決定要 flush 哪幾段——`display_hal_round_area()`(`display.cpp:133`)這個 CO5300 偶數對齊 rounder callback 的存在,證明這個架構本來就設計成能處理局部區域 invalidate,只是 GIF 這條路目前沒用到

**預期效果與已知限制**:多數動畫幀(角色小動作、局部晃動)變動範圍通常只佔畫面一小塊,傳輸量等比例下降,80MHz 加上這個應該能壓到感覺不出 sweep。但畫面內容整體大幅變動的幀(遠景切近景這種硬切鏡頭)bounding box 算出來還是接近整張,**這種幀依然會 sweep**——這是內容本身的性質加上這片板子沒有 TE 同步的物理限制,不是這個改動能解的,不要誤判成沒做好。這個改動不影響現有的 turbo/coalesce 正確性前提(decode 側完全不動,只在 decode 完之後、通知 LVGL 之前多一個 diff 步驟)。
