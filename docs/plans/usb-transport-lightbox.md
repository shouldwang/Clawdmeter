...
- 效能備考(2026-07-10 更新):實機加計時 log 量測,`03_its_fine.gif` 穩定態解碼耗時穩定重現 45ms/90ms(約半個幀預算),`04_oiiai.gif` 遠低於半個預算。試開引擎的 Turbo 模式(`pTurboBuffer`)確實把前者的耗時壓到量不到,但實機播放**立刻出現透明區域花屏**——`DecodeLZWTurbo` 的 COOKED 合成把每條掃描線寫進 `pTurboBuffer` 內部一塊逐行覆寫的 scratch 區、再靠 `pfnDraw` callback 交出,disposal 0/1/3「保留原樣」的透明像素在這條路徑下讀到的是殘留 LZW 字典垃圾,不是真正的前一幀像素(非 turbo 路徑因為直寫持久的 `pFrameBuffer` 才沒有這個問題)。已還原 turbo 相關程式碼,只保留計時 log。結論:turbo 只有在素材先 coalesce/unoptimize(每幀完整獨立、不依賴差分)之後才安全,目前素材都還沒做這個前處理,故不啟用;細節見 `.agent/memory/semantic/LESSONS.md`「AnimatedGIF 的 Turbo 模式跟『保留前一幀透明像素』互斥」條目
- 素材 coalesce 實驗(2026-07-10):兩支 GIF 都是 ezgif.com/optimize 輸出、`disposal asis` + per-frame 透明子矩形的高度差分優化格式,直接 `gifsicle --unoptimize` 會報 "too complex to unoptimize"(local color table/transparency 太複雜)。兩段式才成功:`gifsicle --colors=255` 先把 local palette 併成單一 global palette,再 `gifsicle --unoptimize --lossy=80` 才能把每幀變成完整獨立畫面(`gifsicle --info` 確認每幀都是滿版 240×240、無 transparent/disposal 標記)。代價:檔案暴增到原本的 3.7-4.4 倍(`03_its_fine.gif` 163KB→608KB,`04_oiiai.gif` 120KB→532KB);**更正(2026-07-10 稍晚查證)**:此處原記載 SPIFFS partition 3.375MB(`default_16MB.csv`)有誤——`waveshare_amoled_216` env 並未覆寫 `board_build.partitions`(對照 platformio.ini 其他三個 env 都有設 `default_16MB.csv`,唯獨此 env 沒設),實際 build 出的 `partitions.bin` 解析結果沿用 ESP32-S3 預設 `default_8MB.csv`:**spiffs 1.5MB @ 0x670000**。coalesce 後兩支 GIF 總量約 1.14MB(608KB+532KB)+ PNG×2(~5KB),1.5MB 裝得下,但 headroom 只剩約 300KB,這是為了讓 turbo 模式安全所付出的空間成本,還沒驗證換來的解碼加速划不划算。**已改用原始素材上傳回裝置,coalesce 版本只留在 scratchpad,尚未套用**——下一步排到之後:重新上傳 coalesce 素材 + 重開 turbo(帶正確的 `pfnDraw` scanline-relay callback),實機驗證畫面正確且解碼確實變快才算數。
