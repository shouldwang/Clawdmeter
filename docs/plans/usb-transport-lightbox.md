...
2. **統一三處重複的畫面切換邏輯**:`ui.cpp` 的 `global_click_cb`、`ui_toggle_splash`、`ui_cycle_screen` 三個函式 body 逐字重複(splash ↔ prev_non_splash_screen toggle)。lightbox 加入循環前必須先收斂成單一實作,否則三處漏改一處就靜默分岔(Phase 2+3 驗收 finding)
3. **修 screenshot.sh 在 TinyUSB CDC 下的截斷**:`ARDUINO_USB_MODE=0` 後 `send_screenshot()` 一次寫 460KB 會穩定卡在約 207KB。修法:韌體端分塊寫入 + flush(或調 USBCDC TX timeout)。與 Phase 4 同批修、同次燒錄
4. **清理死碼**:`hal/power_hal.h` 的 `power_hal_pwr_long_pressed`/`power_hal_pwr_released` 與各板 `power.cpp` 對應實作,在 pair_tick 拆除後零呼叫者(Phase 2+3 驗收 finding)。若 Phase 4 不重用長按手勢就連宣告一起移除
